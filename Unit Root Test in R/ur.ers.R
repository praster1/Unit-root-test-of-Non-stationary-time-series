ur.ers = function (y, type = c("DF-GLS", "P-test"), model = c("constant", 
    "trend"), lag.max = 4) 
{
    type = match.arg(type)
    ...
    what = function(x, z = y) {
        ...
        data.what = data.frame(cbind(z.diff[-(1:(x - 1))], z.l[-(1:(x - 
            1))], z.dlags))
    ...
    if (type == "P-test") {
        ...
        if (lag.max > 1) {
            ...
            data.what = data.frame(cbind(y.diff[-(1:(BIC.opt - 
                1))], y.l[-(1:(BIC.opt - 1))], y.dlags))
        ...
        if (model == "constant") {
            cvals = as.matrix(t(c(-2.5658 - 1.96/nobs - 10.04/(nobs^2), 
                -1.9393 - 0.398/nobs, -1.6156 - 0.181/nobs)))
            model = "with intercept"
        }
        else if (model == "trend") {
            cvals.dfgls.tau = matrix(-1 * c(3.77, 3.58, 3.46, 
                3.48, 3.19, 3.03, 2.93, 2.89, 2.89, 2.74, 2.64, 
                2.57), nrow = 4, ncol = 3)
    ...
    new("ur.ers", y = y, yd = yd, type = type, model = model, 
        lag = as.integer(lag.max), cval = round(cvals, 2), teststat = teststat, 
        testreg = test.reg, test.name = "Elliot, Rothenberg and Stock")
}